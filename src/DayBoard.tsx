...
import { CheckSquare, Square, Trash2, Plus, Utensils, Droplets, ListTodo, X, RotateCcw, Calendar, Scale, ArrowUp, ArrowDown, Minus, BookOpen } from "lucide-react";
...
export default function DayBoard() {
  ...
  const [foodForm, setFoodForm] = useState({ name: "", calories: "", protein: "", fat: "" });
  ...
  const addTask = () => {
    ...
    const next = [...tasks, { id: uid(), text, done: false, createdAt: Date.now(), due }];
    persistTasks(next);
    setNewTaskText("");
    setDueDateInput("");
    setDueTimeInput("");
  ...
  const persistFood = useCallback(async (next) => {
    setFoodEntries(next);
    await saveKey(`food:${dateKeyRef.current}`, next);
  }, []);
  ...
  const addFood = () => {
    ...
    setFoodForm({ name: "", calories: "", protein: "", fat: "" });
  };
  ...
  const persistWater = useCallback(async (next) => {
    ...
    await saveKey(`water:${dateKeyRef.current}`, next);
  }, []);
...
function TasksView({
  tasks, newTaskText, setNewTaskText, addTask, toggleTask, deleteTask, clearDoneTasks,
...
function TaskRow({ task, onToggle, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl px-3 py-2.5" style={{ background: palette.surface, border: `1px solid ${palette.border}` }}>
      <button onClick={onToggle} style={{ color: task.done ? palette.tasksAccent : palette.mutedInk }}>
        {task.done ? <CheckSquare size={20} /> : <Square size={20} />}
      </button>
      <div className="flex-1">
        <span
          className="text-sm block"
          style={{ color: task.done ? palette.mutedInk : palette.ink, textDecoration: task.done ? "line-through" : "none" }}
        >
          {task.text}
        </span>
...
function FoodView({
  ...
  addProductToLibrary, deleteProductFromLibrary, logProductFromLibrary,
}) {
  return (
    <div>
      <Card>
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="text-xs flex items-center gap-1 rounded-lg px-2 py-1"
        ...
        {showLibraryForm && (
          <div className="space-y-2 mb-3 pb-3" style={{ borderBottom: `1px solid ${palette.border}` }}>
            <input
        ...
        {savedFoods.length === 0 ? (
          <p className="text-xs text-center py-1" style={{ color: palette.mutedInk }}>
      ...
      <Card>
        <p className="text-sm font-medium mb-3">הוספה חד-פעמית</p>
        <div className="space-y-2">
          <input
            value={foodForm.name}
          ...
          {foodEntries.map((f) => (
            <div key={f.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5" style={{ background: palette.surface, border: `1px solid ${palette.border}` }}>
              <div className="flex-1">
                ...
                <p className="text-[11px]" style={{ color: palette.mutedInk }}>
                  {f.time} · {f.calories} קל׳ · {f.protein} ג חלבון · {f.fat} ג שומן
                </p>
              </div>
